# MVP Certification Fixes Implementation Plan

## Overview

Close the 3 gaps found by the 10xDevs MVP certification check (`.claude/prompts/mvp-check.md`):
missing per-item Delete for journal entries/habits/check-ins (CRUD criterion), no
`context/foundation/test-plan.md` mapping the existing test suite to defined risks (Tests
criterion), and no root `README.md` (Documentation criterion).
...
`SELECT`/`INSERT`/`UPDATE` — a remote delete call would silently affect 0 rows today. `context/`
has no `test-plan.md`, and the repo has no root `README.md` at all (`context/foundation/README.md`
only documents foundation-doc conventions, not the project).
...
the network and never blocks on it. `context/foundation/test-plan.md` names the risks the test suite
(existing + new) addresses. A root `README.md` describes the project for a reader who opens the repo
cold.

Verification: re-run the `/10x-health-check`-style checks (`ktlintCheck`, `testDebugUnitTest`,
`assembleDebug`) all green; manually delete one of each entity type in a running build and confirm
it disappears from the main screen and does not return after a manual sync (signed-in device).
...
## Phase 5: `context/foundation/test-plan.md`

### Overview

Write a foundation doc mapping the existing ~45 unit test files, plus the new delete-feature tests
from Phases 3-4, to the concrete risks they cover — closing the certification's "Tests addressing a
defined risk" gap for the whole suite, not just the new code.

### Changes Required:

#### 1. New foundation doc

**File**: `context/foundation/test-plan.md`

**Intent**: For each risk category, name the risk, why it matters (tie back to a PRD requirement or
an architectural constraint where one exists), and which test file(s) exercise it. Cover at minimum:
24h edit-window enforcement (`EditWindowTest`, `UpdateJournalEntryUseCaseTest`,
`UpdateHabitCheckInUseCaseTest`), contribution-level calculation correctness
(`HabitContributionCalculatorTest`, `JournalContributionCalculatorTest`, `ContributionWindowTest`,
`ContributionMapperTest`), local persistence correctness (`JournalEntryDaoTest`, `HabitDaoTest`,
`HabitCheckInDaoTest`, `TodayWasDatabaseTest`), sync/auth failure handling
(`JournalRepositoryImplTest`, `HabitRepositoryImplTest`, `OnboardingRepositoryImplTest`,
`AuthErrorMapperTest`, `AuthStateMapperTest`, `SignOutUseCaseTest`, `SyncLocalDataUseCaseTest`), and
the new delete-specific risks from this change: a delete succeeding regardless of entry age (by
design, unlike Update), a habit delete correctly cascading to its check-ins locally, and a local
delete not being resurrected by a subsequent sync (the correctness risk identified in Key
Discoveries above) — all three covered by the new tests added in Phases 3-4.

**Contract**: Follows `context/foundation/README.md`'s stated conventions (edit-in-place, no dated
copies) since it's a new foundation doc, not a change-scoped one — this is what outlives the current
change, unlike `plan.md`/`plan-brief.md` in this same folder.

### Success Criteria:

#### Automated Verification:

- File exists at `context/foundation/test-plan.md` and is valid Markdown

#### Manual Verification:

- Every risk named in the doc points to at least one real, currently-passing test file
- Re-running the certification check (`.claude/prompts/mvp-check.md`, criterion 3) against the
  updated repo would mark it ✅

---

## Phase 6: Root `README.md`

### Overview

Add the root `README.md` the certification check's Documentation criterion expects but doesn't
currently exist anywhere in the repo.
...
**Intent**: Give a reader who opens the repo cold (e.g. a certification grader) what the project is,
its core features (journaling, habit tracking, contribution-style history, optional account sync,
AI-assisted prompts), the module layout (`:app` + `:core:designsystem`, pointing at `CLAUDE.md` for
the full convention set rather than duplicating it), and the build/test commands already listed in
`CLAUDE.md`'s "Build, Test, and Development Commands" section.

**Contract**: Standard top-level sections (What is this / Features / Module structure / Build & test
/ Where to learn more, pointing at `context/foundation/prd.md` and `CLAUDE.md`). No screenshots or
visual-design content — out of scope per the certification check's own instructions ("visual
design... reviewed separately").

### Success Criteria:

#### Automated Verification:

- File exists at `README.md` (repo root) and is valid Markdown

#### Manual Verification:

- Re-running the certification check (criterion 5) against the updated repo would mark it ✅
- A reader unfamiliar with the project can tell what it is and how to build it from the README
  alone, without opening any other file
...
- [ ] 1.1 Migration applies cleanly via `mcp__supabase__apply_migration`
- [ ] 1.2 `pg_policies` query confirms all three `*_delete_own` policies present

#### Manual

- [ ] 1.3 Explicit user go-ahead obtained before applying (production database change)
- [ ] 1.4 `mcp__supabase__get_advisors` shows no new security warnings
...
- [ ] 5.1 File exists at `context/foundation/test-plan.md`, valid Markdown

#### Manual

- [ ] 5.2 Every named risk points to a real, passing test file
- [ ] 5.3 Re-run of certification criterion 3 would mark ✅

### Phase 6: Root `README.md`

#### Automated

- [ ] 6.1 File exists at `README.md`, valid Markdown

#### Manual

- [ ] 6.2 Re-run of certification criterion 5 would mark ✅
- [ ] 6.3 Unfamiliar reader understands the project and how to build it from the README alone

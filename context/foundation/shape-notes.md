...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: "insight"
      decision:
        "presence over streaks — a contribution-style history shows consistency without punishing
        gaps, unlike streak counters"
    - topic: "persona scope"
      decision:
        "people who want to start or maintain a daily journaling habit, and/or want to stick to
        habits they're tracking"
    - topic: "auth strategy"
      decision:
        "usable without login (local, on-device data); optional account via email or Google sign-in
        to save/sync"
    - topic: "role model"
      decision: "flat — no roles, identical capabilities regardless of signed-in status"
    - topic: "mvp flow scope"
      decision:
        "onboarding (focus pick) → add habit → add daily entry → view on main screen → edit;
